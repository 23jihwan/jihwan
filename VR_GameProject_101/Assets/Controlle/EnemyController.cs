...
public class EnemyController : MonoBehaviour
{
    ...
    void Update()
    {
        if(player != null)
        {
            ...
            {
                Vector3 direction = (player.position - transform.position).normalized;       //플레이어와 나와 위치 백터를 뺄샘하여 방향 백터를 설정
                rb.MovePosition(transform.position + direction * speed * Time.deltaTime);    //적 이동
            }

            //포탑 회전
            Vector3 targetDirection = (player.position - EnemyPivot.transform.position).normalized;     //플레이어와 적 피봇 백터를 뺄셈하여 방향 백터를 설정
            Quaternion targetRotate = Quaternion.LookRotation(targetDirection);     //회전 값을 정해서
            EnemyPivot.transform.rotation =
                Quaternion.Lerp(EnemyPivot.transform.rotation, targetRotate, rotationSpeed * Time.deltaTime); //회전 시켜주는 함수
